              ...
              <table class="table table-flush table-hover table-striped" id="datatable-buttons">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      {{ ucwords(Str::limit($product->user->name, 18)) }}
                    </td>
                   <td>{{ $product->updated_at->format('Y-m-d') }}</td>
                    <td class="table-action">
                      <a href="{{route('products.edit', $product)}}" class="table-action" data-toggle="tooltip"
                        data-original-title="Edit product">

                        <i class="fas fa-user-edit"></i>
                      </a>
                      <a href="#!" class="table-action table-action-delete" data-toggle="modal"
                        data-target="#delete{{$product->id}}" data-original-title="Delete product">
                        <i class="fas fa-trash"></i>
                      </a>
                    </td>
                    {{-- delete modal --}}
                    <div class="modal fade" tabindex="-1" role="dialog" id="delete{{$product->id}}">
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            <p>Are you sure you wanted to delete this Product?.</p>
                          </div>
                          <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                            <form method="POST" , action="{{ route('products.destroy', $product)}}" class="">
                              @csrf
                              @method('DELETE')
